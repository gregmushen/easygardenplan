...
suite("planning persistence and concurrency", () => {
  const nonce = crypto.randomUUID(); const organizationId = `planning-${nonce}`; const gardenId = id(); const cropId = id(); const bedId = id(); const revisionId = id(); const sourceId = id(); const evidenceId = id(); const familyIds = [id(), id()]; const ruleIds = [id(), id()]; const releaseId = id();
  ...
  afterAll(async () => {
    ...
    await database!.delete(ruleVersion).where(sql`${ruleVersion.id} in (${ruleIds[0]}, ${ruleIds[1]})`);
    await database!.delete(ruleFamily).where(sql`${ruleFamily.id} in (${familyIds[0]}, ${familyIds[1]})`);
