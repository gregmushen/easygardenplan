...
export class NotificationRepository {
  ...
  async claim(transitionId: string, leaseMilliseconds = 60_000): Promise<ClaimedNotification | null> {
    return await this.database.transaction(async (transaction) => {
      ...
      const enabled = details?.deliveryClass === "routine_digest" ? (preference?.routineEmailEnabled ?? true) && (preference?.digestEmailEnabled ?? true) : details?.kind === "resolution" ? (preference?.resolutionEmailEnabled ?? true) : (preference?.urgentEmailEnabled ?? true);
