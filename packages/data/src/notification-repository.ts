import { garden, notificationDeliveryIntent, notificationPreference, recommendationTransition, recommendationVersion, user, type Database } from "@easygardenplan/db";
...
export class NotificationRepository {
  ...
  async claim(transitionId: string, leaseMilliseconds = 60_000): Promise<ClaimedNotification | null> {
    return await this.database.transaction(async (transaction) => {
      ...
      const [details] = await transaction.select({ recipient: user.email, emailVerified: user.emailVerified, gardenName: garden.name, kind: recommendationTransition.kind, action: recommendationVersion.action, validFrom: recommendationVersion.validFrom, validThrough: recommendationVersion.validThrough }).from(notificationDeliveryIntent).innerJoin(user, eq(user.id, notificationDeliveryIntent.recipientUserId)).innerJoin(garden, and(eq(garden.organizationId, notificationDeliveryIntent.organizationId), eq(garden.id, notificationDeliveryIntent.gardenId))).innerJoin(recommendationTransition, and(eq(recommendationTransition.organizationId, notificationDeliveryIntent.organizationId), eq(recommendationTransition.id, notificationDeliveryIntent.transitionId))).innerJoin(recommendationVersion, and(eq(recommendationVersion.organizationId, notificationDeliveryIntent.organizationId), eq(recommendationVersion.id, notificationDeliveryIntent.recommendationVersionId))).where(eq(notificationDeliveryIntent.id, claimed.id)).limit(1);
      ...
      if (!details?.emailVerified || !enabled) {
        await transaction.update(notificationDeliveryIntent).set({ status: "suppressed", suppressionReason: details?.emailVerified ? "preference_disabled" : "recipient_unverified", attemptToken: null, leaseExpiresAt: null, updatedAt: now }).where(and(eq(notificationDeliveryIntent.id, claimed.id), eq(notificationDeliveryIntent.attemptToken, token)));
        return null;
      }
