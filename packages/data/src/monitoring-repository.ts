...
export class MonitoringRepository {
  ...
  async evaluate(input: { gardenId: string; hazard: "cold" | "heat" | "official_alert"; groupKey: string; observation: RiskObservation; snapshotId?: string; event?: TransitionEventFactory; resolutionConfirmations?: number }) {
    return await this.database.transaction(async (transaction) => {
      ...
      const [priorRecommendation] = candidate ? [] : await transaction.select({ deliveryClass: recommendationVersion.deliveryClass, affectedIds: recommendationVersion.affectedIds, affectedCropNames: recommendationVersion.affectedCropNames, affectedTaskIds: recommendationVersion.affectedTaskIds }).from(recommendationVersion).where(and(eq(recommendationVersion.organizationId, this.organizationId), eq(recommendationVersion.episodeId, decision.episodeId))).orderBy(desc(recommendationVersion.version)).limit(1);
      const deliveryClass = candidate?.deliveryClass ?? priorRecommendation?.deliveryClass ?? "urgent";
      const selectionIds = (candidate?.affectedIds ?? []).filter((id) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iu.test(id));
      const affectedCrops = selectionIds.length ? await transaction.select({ name: crop.commonName }).from(cropSelection).innerJoin(crop, eq(crop.id, cropSelection.cropId)).where(and(eq(cropSelection.organizationId, this.organizationId), eq(cropSelection.gardenId, input.gardenId), inArray(cropSelection.id, selectionIds))) : [];
      const affectedCropNames = candidate ? [...new Set(affectedCrops.map(({ name }) => name))].sort() : priorRecommendation?.affectedCropNames ?? [];
      const resolutionAction = input.hazard === "official_alert" ? "The official weather alert for this period is no longer active." : "The forecast risk for this period has passed.";
      const [recommendation] = await transaction.insert(recommendationVersion).values({ organizationId: this.organizationId, gardenId: input.gardenId, episodeId: decision.episodeId, version: priorVersion + 1, transitionKind: decision.transition, deliveryClass, action: candidate?.action ?? (decision.transition === "resolution" ? resolutionAction : null), affectedIds: candidate?.affectedIds ?? priorRecommendation?.affectedIds ?? [], affectedCropNames, affectedTaskIds: candidate?.affectedTaskIds ?? priorRecommendation?.affectedTaskIds ?? [], validFrom: candidate ? new Date(candidate.validFrom) : null, validThrough: candidate ? new Date(candidate.validThrough) : null, evaluationId: evaluation.id, evidence: { snapshotId: input.snapshotId ?? null, observation: input.observation }, createdAt: now }).returning();
  ...
  async listOfficialAlerts(gardenId: string) { return await this.database.select({ id: weatherOfficialAlertSnapshot.id, event: weatherOfficialAlertSnapshot.event, status: weatherOfficialAlertSnapshot.status, messageType: weatherOfficialAlertSnapshot.messageType, sentAt: weatherOfficialAlertSnapshot.sentAt, effectiveAt: weatherOfficialAlertSnapshot.effectiveAt, onsetAt: weatherOfficialAlertSnapshot.onsetAt, expiresAt: weatherOfficialAlertSnapshot.expiresAt, endsAt: weatherOfficialAlertSnapshot.endsAt, cancelled: weatherOfficialAlertSnapshot.cancelled, headline: weatherOfficialAlertSnapshot.headline, sourceUrl: weatherOfficialAlertSnapshot.sourceUrl, areaDescription: weatherOfficialAlertSnapshot.areaDescription }).from(weatherOfficialAlertGarden).innerJoin(weatherOfficialAlertSnapshot, eq(weatherOfficialAlertSnapshot.id, weatherOfficialAlertGarden.alertSnapshotId)).where(and(eq(weatherOfficialAlertGarden.organizationId, this.organizationId), eq(weatherOfficialAlertGarden.gardenId, gardenId))).orderBy(desc(weatherOfficialAlertSnapshot.sentAt)).limit(20); }
}
