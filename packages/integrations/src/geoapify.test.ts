...
describe("Geoapify adapter", () => {
  it("normalizes candidates and keeps the key out of returned data", async () => {
    const providerFetch = vi.fn(async (request: URL | RequestInfo) => {
      ...
      return new Response(JSON.stringify({ results: [{ place_id: "place-1", formatted: "Portland, OR", lat: 45.52, lon: -122.68, result_type: "city", rank: { confidence: 0.95 }, timezone: { name: "America/Los_Angeles" } }] }), { status: 200 });
    });
    const result = await new GeoapifyGeocoder({ apiKey: "secret-fixture", fetch: providerFetch }).forward({ text: "Portland, OR", limit: 3 });
    expect(result).toEqual([expect.objectContaining({ providerPlaceId: "place-1", timezone: "America/Los_Angeles", confidence: 0.95 })]);
