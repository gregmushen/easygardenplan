...
export const ruleVersion = pgTable("rule_version", {
  id: uuid("id").defaultRandom().primaryKey(), familyId: uuid("family_id").notNull().references(() => ruleFamily.id, { onDelete: "restrict" }), version: integer("version").notNull(), schemaVersion: integer("schema_version").default(1).notNull(), state: text("state").default("draft").notNull(), applicability: jsonb("applicability").notNull(), payload: jsonb("payload").notNull(), evidenceIds: jsonb("evidence_ids").notNull(), reviewDecisionId: uuid("review_decision_id").references(() => reviewDecision.id, { onDelete: "restrict" }), replacementVersionId: uuid("replacement_version_id"), publishedAt: timestamp("published_at", { withTimezone: true }), withdrawnAt: timestamp("withdrawn_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).defaultNow().notNull(),
