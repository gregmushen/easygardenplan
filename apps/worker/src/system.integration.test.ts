...
suite("local product path", () => {
  it("verifies email, signs in, selects an organization, and reads tenant billing", async () => {
    ...
    try {
      ...
      const confirmedLocation = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/location`, { method: "PUT", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify({ expectedRevision: 2, location: { coordinate: { latitude: 45.52, longitude: -122.68 }, timezone: "America/Los_Angeles", source: "manual_pin" } }) }, environment);
      expect(confirmedLocation.status).toBe(200);
      await expect(confirmedLocation.json()).resolves.toMatchObject({ garden: { revision: 3, locationConfirmed: true, locationSource: "manual_pin", latitude: "45.52", longitude: "-122.68" } });
      const staleLocation = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/location`, { method: "PUT", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify({ expectedRevision: 2, location: { coordinate: { latitude: 1, longitude: 1 }, timezone: "Etc/UTC", source: "manual_pin" } }) }, environment);
      expect(staleLocation.status).toBe(409);
      const associatedClimate = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/climate/associate`, { method: "POST", headers: { ...gardenHeaders, "content-type": "application/json" }, body: "{}" }, environment);
      expect(associatedClimate.status).toBe(200);
      await expect(associatedClimate.json()).resolves.toMatchObject({ climate: { gardenId: workspace.gardenId, source: expect.stringMatching(/^(dataset_match|unavailable)$/u) } });
      const userAnchor = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/climate/anchor`, { method: "PUT", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify({ hardinessZone: "8b", frostState: "known", springFrostLocalDate: "03-15", autumnFrostLocalDate: "11-10", rationale: "Dates from my garden journal" }) }, environment);
      expect(userAnchor.status).toBe(200);
      await expect(userAnchor.json()).resolves.toMatchObject({ climate: { source: "user_anchor", state: "known", springFrostLocalDate: "03-15" } });
      const currentClimate = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/climate`, { headers: gardenHeaders }, environment);
      expect(currentClimate.status).toBe(200);
      await expect(currentClimate.json()).resolves.toMatchObject({ climate: { source: "user_anchor", state: "known" } });
      const bedInput = { name: "North Bed", expectedRevision: 0, geometry: { outer: [{ x: 0, y: 0 }, { x: 4, y: 0 }, { x: 4, y: 3 }, { x: 2, y: 2 }, { x: 0, y: 3 }], exclusions: [[{ x: 0.5, y: 0.5 }, { x: 1, y: 0.5 }, { x: 1, y: 1 }]] }, transform: { anchor: { latitude: 45.52, longitude: -122.68 }, rotationRadians: 0, scale: 1, translationMeters: { x: 0, y: 0 }, projection: "local_equirectangular_v1" }, measurementProvenance: "measured", sunlight: [{ observedOn: "2026-06-15", hours: 7, category: "full_sun" }] };
      const createdBed = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/beds`, { method: "POST", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify(bedInput) }, environment);
      expect(createdBed.status).toBe(201);
      const createdBedBody = await createdBed.json() as { bedId: string; revision: { revision: number } };
      expect(createdBedBody.revision.revision).toBe(1);
      const updatedBed = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/beds/${createdBedBody.bedId}`, { method: "PUT", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify({ ...bedInput, expectedRevision: 1, name: "Measured North Bed" }) }, environment);
      expect(updatedBed.status).toBe(200);
      expect((await updatedBed.json() as { revision: { revision: number } }).revision.revision).toBe(2);
      expect((await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/beds/${createdBedBody.bedId}`, { method: "PUT", headers: { ...gardenHeaders, "content-type": "application/json" }, body: JSON.stringify({ ...bedInput, expectedRevision: 1 }) }, environment)).status).toBe(409);
      const bedHistory = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/beds/${createdBedBody.bedId}/history`, { headers: gardenHeaders }, environment);
      expect((await bedHistory.json() as { revisions: unknown[] }).revisions).toHaveLength(2);
      const bedPrint = await app.request(`http://localhost:8787/api/gardens/${workspace.gardenId}/beds/${createdBedBody.bedId}/print.svg`, { headers: gardenHeaders }, environment);
      expect(bedPrint.status).toBe(200);
      const svg = await bedPrint.text();
      expect(svg).toContain("Measured North Bed");
