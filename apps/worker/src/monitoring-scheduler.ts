...
export async function scheduleDueWeatherEvaluations(environment: AuthEnvironment, clock: { now(): Date } = { now: () => new Date() }): Promise<{ claimed: number; queued: number; failed: number }> {
  const now = clock.now(); const platform = createPlatformDatabase(environment.DATABASE_URL, environment.DATABASE_DRIVER); const claimed: Array<{ gardenId: string; organizationId: string; locationRevision: number; token: string }> = [];
  try {
    await platform.transaction(async (transaction) => {
      const rows = await transaction.select().from(weatherLocationDue).where(and(lte(weatherLocationDue.nextDueAt, now), or(isNull(weatherLocationDue.leaseExpiresAt), lt(weatherLocationDue.leaseExpiresAt, now)))).orderBy(asc(weatherLocationDue.nextDueAt)).limit(50).for("update", { skipLocked: true });
      for (const row of rows) { const token = crypto.randomUUID(); await transaction.update(weatherLocationDue).set({ leaseToken: token, leaseExpiresAt: new Date(now.getTime() + 5 * 60_000), updatedAt: now }).where(and(eq(weatherLocationDue.gardenId, row.gardenId), eq(weatherLocationDue.organizationId, row.organizationId))); claimed.push({ gardenId: row.gardenId, organizationId: row.organizationId, locationRevision: row.locationRevision, token }); }
    });
    ...
    for (const item of claimed) {
      try {
        ...
        await platform.update(weatherLocationDue).set({ nextDueAt: new Date(now.getTime() + 60 * 60_000), leaseToken: null, leaseExpiresAt: null, lastSucceededAt: now, failureCount: 0, updatedAt: now }).where(and(eq(weatherLocationDue.gardenId, item.gardenId), eq(weatherLocationDue.leaseToken, item.token))); queued++;
      } catch {
        await platform.update(weatherLocationDue).set({ nextDueAt: new Date(now.getTime() + 5 * 60_000), leaseToken: null, leaseExpiresAt: null, failureCount: 1, updatedAt: now }).where(and(eq(weatherLocationDue.gardenId, item.gardenId), eq(weatherLocationDue.leaseToken, item.token))); failed++;
      }
    }
