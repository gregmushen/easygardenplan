...
type ClimateSummary = { state: "known" | "frost_free" | "unknown" | "uncertain"; hardinessZone: string | null; springFrostLocalDate: string | null; autumnFrostLocalDate: string | null; confidence: number; rationale: string; source: "dataset_match" | "user_anchor" | "unavailable"; sourceEvidence?: Array<{ kind: "hardiness" | "frost_normals" | "combined_fixture"; sourceName: string; sourceRelease: string; attribution: string }> };
...
export function LocationSetup(props: { garden: Garden; organizationId: string; onUpdated: (garden: Garden) => void }) {
  ...
  return <section className="mt-10 border-t border-slate-200 pt-8">
    ...
    <div className="mt-8 rounded-xl border border-slate-200 p-5"><h3 className="font-semibold">Seasonal context</h3>{climate === undefined ? <p className="mt-2 text-sm text-slate-500">Checking climate information…</p> : climate ? <div className="mt-2 text-sm text-slate-700"><p>Status: <strong>{climate.state.replace("_", " ")}</strong>{climate.hardinessZone ? ` · Zone ${climate.hardinessZone}` : ""}</p><p className="mt-1">{climate.rationale}</p>{climate.state === "known" && <p className="mt-1">Typical anchors: spring {climate.springFrostLocalDate}, autumn {climate.autumnFrostLocalDate}</p>}{climate.sourceEvidence && climate.sourceEvidence.length > 0 && <div className="mt-3 border-t border-slate-200 pt-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Climate sources</p><ul className="mt-1 space-y-1 text-xs">{climate.sourceEvidence.map((source) => <li key={`${source.kind}:${source.sourceRelease}`}>{source.attribution} · {source.sourceRelease}</li>)}</ul></div>}</div> : <p className="mt-2 text-sm text-slate-600">No climate association has been saved yet.</p>}
