...
function BillingSettings() {
  ...
  const subscription = useQuery({ queryKey: billingSubscriptionQueryKey(session?.user.id, organizationId), enabled: Boolean(session?.user.id && organizationId), queryFn: async () => { const response = await fetch(api("/api/billing/subscription"), { credentials: "include", headers: { "x-trestle-tenant": organizationId! } }); if (!response.ok) throw new Error("Sign in before managing billing"); return response.json() as Promise<{ subscription: null | { plan: string; planVersion: number; status: string; currentPeriodEnd?: string; cancelAtPeriodEnd: boolean; entitlements: string[]; effectiveEntitlements?: Array<{ code: string; enabled: boolean; source: string; inheritedFrom?: string }> }; usage: Array<{ meter: string; used: number; limit: number | null }> }>; } });
  const [message, setMessage] = useState<string>();
