...
- Monitoring eligibility is updated atomically when a garden's location or monitoring preference changes. Gardens remain due hourly, while a separate application cron drains up to 50 due gardens every two minutes. Trestle's framework minute tick remains separate.
- The declared 1,000-active-garden pilot model requires 1,000 evaluations per hour. The scheduler provides 1,500 slots per hour with five-way bounded tenant work and capped exponential storage-failure backoff. The conservative provider model is recorded in `monitoring-capacity-evidence.md`.
