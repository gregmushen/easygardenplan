...
Status: **in progress**. The complete deterministic local path is implemented and verified. The phase remains open for explicit geometry-reconciliation conflicts, a customer-facing correction flow, accessibility/offline review, and validation against the real reviewed catalog and climate data tracked in phases 2 and 3.
...
- Surface reconciliation conflicts when a later geometry revision no longer contains an actual planting position.
- Add the customer-facing correction interaction over the implemented append-only correction contract.
