...
const server = repl.start({ prompt: `easygardenplan [${environment}${resolvedTenant ? ` tenant=${resolvedTenant.slug}` : " platform"} ${mode}]> `, useGlobal: false, ignoreUndefined: true });
