...
type Artifact = {
  ...
  candidates: Array<{ cropSlug: string; commonName: string; ruleType: string; methods: string[]; regionIds: string[]; payload: unknown; normalizedClaim: string; originalUnits: string; locator: string }>;
};
...
if (artifact.schemaVersion !== 1 || artifact.normalizationVersion !== 1 || artifact.status !== "editorial_candidates" || !artifact.source?.contentChecksumSha256 || !Array.isArray(artifact.candidates)) throw new Error("Unsupported candidate artifact");
...
const contextKey = `norm${artifact.normalizationVersion}-${artifact.source.contentChecksumSha256.slice(0, 24)}`;
let cropsCreated = 0;
let draftsCreated = 0;
let skipped = 0;
...
for (const candidate of artifact.candidates) {
  ...
  if (!plant) {
    [plant] = await database.insert(crop).values({ slug: candidate.cropSlug, commonName: candidate.commonName, status: "draft" }).onConflictDoNothing().returning();
  ...
  for (const method of candidate.methods) {
    ...
    await repository.createDraft({
      ...
      source: { url: artifact.source.url, title: artifact.source.title, publisher: artifact.source.publisher, sourceType: artifact.source.sourceType, accessedAt: new Date(artifact.source.accessedAt), contentChecksum: artifact.source.contentChecksumSha256, attributionNotes: "Imported as a source-scoped editorial candidate; publication requires separate accepted review." },
...
process.stdout.write(`${JSON.stringify({ cropsCreated, draftsCreated, skipped, contextKey })}\n`);
