...
try {
  ...
  try {
    ...
    const requiredColumns = await client`select table_name, column_name from information_schema.columns where table_schema='public' and (
      ...
      (table_name='climate_association' and column_name='source_evidence') or
      (table_name='garden' and column_name='region_ids')
    ...
    if (requiredColumns.length !== 6) throw new Error("The upgraded schema is missing required post-baseline columns");
